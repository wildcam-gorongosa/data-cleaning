...
missingdata <- "https://panoptes-uploads.zooniverse.org/production/subject_location/94a3fa73-851b-4c51-b308-f76093aaaeea.jpeg"
...
# expert IDs 
rare.spp <- read.csv("rare_spp_urls_kg.csv")
rare.spp <- rare.spp[c("capture_id", "subject_id", "question__species", "kg_species")]
names(rare.spp) <- c("filename", "directory", "species", "new_species")
rare.spp$species <- as.character(rare.spp$species)
rare.spp$new_species <- as.character(rare.spp$new_species)
rare.spp <- rare.spp[!(rare.spp$species == rare.spp$new_species),]
rare.spp[rare.spp$species == "wilddog",]$species <- "wild_dog"

rare.spp$expert.id.date <- "29-Nov-2021"
rare.spp$filename <- paste("capture_id:", rare.spp$filename)
rare.spp$directory <- paste("subject_id:", rare.spp$directory)
rare.spp$tag <- paste(rare.spp$filename, rare.spp$directory, rare.spp$species)
nrow(rare.spp) #100

x$tag <- paste(x$filename, x$directory, x$species)
y <- x[x$tag %in% unique(rare.spp$tag),]; nrow(y) #100
test <- merge(y, rare.spp)
names(test)[c(1,20)] <- c("old_species", "species")

x <- x[!(x$tag %in% unique(rare.spp$tag)),]
nrow(x) #should be 345022
x$tag <- NULL; test$tag <- NULL; x$expert.id.date <- NA; x$old_species <- NA
x <- rbind(x, test)
nrow(x) #should be 345122 
